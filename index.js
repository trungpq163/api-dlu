...
const termImportRegex = new RegExp(
 ...
 */
async function performSyncScheduleFunctions(
    idImport,
    yearImport,
    termImport,
    weekImport
    ...
    if (idImport.indexOf(".") !== -1) {
        let url =
            await `http://online.dlu.edu.vn/Home/DrawingProfessorSchedule?YearStudy=${
                yearImportRegex.test(yearImport) ? yearImport : yearStudy
            }&TermID=${
                termImportRegex.test(termImport) ? termImport : termID
            }&Week=${
                typeof Number(weekImport) === "number" &&
                Number(weekImport) >= 0
                    ? weekImport
                    : week
        ...
        (typeof Number(idImport) === "number") &
        (Number(idImport) > 0)
    ) {
        let url =
            await `http://online.dlu.edu.vn/Home/DrawingStudentSchedule?StudentId=${Number(
                idImport
            )}&YearId=${
                yearImportRegex.test(yearImport) ? yearImport : yearStudy
            }&TermId=${
        ...
        let url =
            await `http://online.dlu.edu.vn/Home/DrawingClassStudentSchedules_Mau2?YearStudy=${
                yearImportRegex.test(yearImport) ? yearImport : yearStudy
            }&TermID=${
                termImportRegex.test(termImport) ? termImport : termID
            }&Week=${
